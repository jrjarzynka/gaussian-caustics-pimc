...
# LHA-13B3B
#
# Physical-bead histogram on exactly the same reduced-coordinate
# bins as the analytically bin-integrated exact target.
#
# Exact target array convention:
#     axis 0 = t bin
#     axis 1 = s bin
#
# Therefore histogram2d is called as histogram2d(t, s).
#
# Beads are pooled for the density point estimator only.
# They are NOT treated as statistically independent for
...
def physical_bead_histogram_ts(
    sample_block,
):
    ...
    flat_block = sample_block.reshape(
        -1,
        2,
    )
...
H_full_ts = physical_bead_histogram_ts(
    samples
)
...
H_first_ts = physical_bead_histogram_ts(
    samples[:mid_saved]
)


H_second_ts = physical_bead_histogram_ts(
    samples[mid_saved:]
)
...
if int(
    ...
    raise RuntimeError(
        "Full physical-bead histogram lost samples."
    )
...
if int(
    ...
    raise RuntimeError(
        "First-half histogram lost samples."
    )
...
if int(
    ...
    raise RuntimeError(
        "Second-half histogram lost samples."
    )


np.savez_compressed(

    f"lhap0a_seed_{SEED}_hist.npz",
...
print(
    "=== B3B PHYSICAL-BEAD HISTOGRAM ==="
)

print(
    f"full bead count   = "
    f"{int(np.sum(H_full_ts))}"
)

print(
    f"first-half count  = "
    f"{int(np.sum(H_first_ts))}"
)

print(
    f"second-half count = "
    f"{int(np.sum(H_second_ts))}"
)
