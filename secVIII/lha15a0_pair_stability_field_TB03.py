...
class BilayerKeldysh:
    ...
    finite separation D makes V(0) finite.  For TB-03 the same optional
    radial wall used by the production PI-QMC Hamiltonian can be included;
    this is essential when evaluating zeta on samples drawn from that
    Hamiltonian.
    """
    ...
    wall_radius_nm: Optional[float] = None
    wall_height_eV: float = 0.0
    wall_power: int = 8
    ...
    def __post_init__(self) -> None:
        ...
        if self.wall_radius_nm is not None:
            if not np.isfinite(self.wall_radius_nm) or self.wall_radius_nm <= 0.0:
                raise ValueError("wall_radius_nm must be positive and finite")
            if not np.isfinite(self.wall_height_eV) or self.wall_height_eV < 0.0:
                raise ValueError("wall_height_eV must be finite and non-negative")
    ...
    def value(self, rho: np.ndarray) -> np.ndarray:
        ...
        v = (-np.pi * self.coulomb_constant_eV_nm
             / (2.0 * self._r0_sum) * (struve(0, x) - yv(0, x)))
        if self.wall_radius_nm is not None and self.wall_height_eV != 0.0:
            v = v + self.wall_height_eV * (
                np.abs(rho) / self.wall_radius_nm
            ) ** self.wall_power
        return v
...
def spectral_Xi(lam: np.ndarray, beta: float) -> np.ndarray:
    """
    Xi_a for mass-weighted eigenvalues lambda_a = omega_a^2.
...
def production_field(amplitude_e_eV: float = 0.045,
                     ...
                     period_nm: float = 20.0,
                     wall_radius_nm: Optional[float] = 15.0,
                     wall_height_eV: float = 0.08,
                     wall_power: int = 8) -> PairStabilityField:
    """
...
def _self_test() -> None:
    print("=" * 70)
    print("A. Landscape and wall off: reproduces the isolated-pair annulus")
    print("=" * 70)
    bare = production_field(amplitude_e_eV=0.0, wall_height_eV=0.0)
    ...
    for r_, v_ in zip(rho, fi):
        tag = "  <- legacy geometric reference only" if abs(r_ - 11.55) < 0.1 else ""
